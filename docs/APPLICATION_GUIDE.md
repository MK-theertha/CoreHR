...
**Document** — added for employee documents and leave-request attachments
([§10](#10-s3--file-storage)). Polymorphic association, same
`(entityType, entityId)` pattern as AuditLog, rather than two near-identical
...
| contentType | string | one of a fixed allowlist — see §10 |
...
| GET/POST | `/employees/me/documents[/upload-url,/confirm]`, `DELETE /employees/me/documents/{docId}` | 🔑 | Own documents — see [§10](#10-s3--file-storage) for the upload flow. 404 (via `get_me`) if caller has no linked Employee. |
...
table — [`Document`](#4-data-model), polymorphically associated via
...
gated to `STAFF_ROLES`/`ADMIN_ROLES` for anyone else's — see [§8](#8-fastapi-backend--complete-api-reference)).
Leave documents are gated by a dedicated visibility check,
`leave_service.get_visible_leave_request` — the request's own employee or a
`CAN_MANAGE` role (SUPER_ADMIN/HR_ADMIN/MANAGER) can see them; anyone else gets
a `404` (not `403`, so the leave request's existence isn't leaked to someone
who shouldn't know about it). Deleting a leave document is further restricted
to while the request is still `PENDING`, unless you're a `CAN_MANAGE` role.
...
leave-document-visibility edge cases. See [§16](#16-testing).
...
- All API endpoints in [§8](#8-fastapi-backend--complete-api-reference), including
  employee documents and leave-request attachments (`Document` table, S3-backed —
  [§10](#10-s3--file-storage)).
...
- Alembic migrations (baseline + one incremental for `Document`, both verified
  round-trip: upgrade → `alembic check` reports zero drift → downgrade).
...
- 144 passing pytest tests (auth dependency, RBAC matrix incl. the document
...
- **The frontend doesn't have UI for the new document endpoints yet** — the
  Employee Detail / Profile pages' Documents tab is still the `EmptyState`
  placeholder noted in [§11](#11-frontend-architecture); the backend capability
  exists (§10) but nothing calls it. Same precedent as profile images, which also
...
- **`test_rbac.py`** — the full permission matrix from [§6](#6-authorization-rbac),
  ...
  disallowed, and *not* `401`/`403` for allowed roles (not strictly `2xx` — a few
  routes, including the document ones with a nonexistent target entity or S3
  unconfigured, correctly return `404`/`400`/`501` for an allowed role too; the
  matrix is testing the permission boundary, not the full business-logic result).
